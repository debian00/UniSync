...
const pauseBookHandler = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json(
        "Exito suspendiendo la publicacion, ahora el libro esta almacenado en las sombras"
      );
  } catch (error) {
    console.log(error);
...
const restoreBookHandler = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json(
        "Exito reanudando la publicacion, el libro vuelve a ver la luz del dia"
      );
  } catch (error) {
    console.log(error);
...
const isForSaleHandler = async (req, res) => {
  const { id } = req.params;
  const { porcentageDiscount } = req.body;
  try {
    const response = await isForSaleController(id, porcentageDiscount);
    res
      .status(200)
      .json({ success: true, message: "Oferta lanzada con éxito", response });
  } catch (error) {
    res.status(400).json({
      success: false,
      message: "Problema al ofertar el libro",
      error: error.message,
    });
  }
};
...
const isNotForSaleHandler = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Oferta cancelada", response });
  } catch (error) {
    res.status(400).json({
      success: false,
      message: "Problema al retirar la oferta",
      error: error.message,
    });
  }
};
