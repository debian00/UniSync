...
const getAllBooksController = async (req) => {
  ...
  try {
    ...
    if (sellPriceMin && !Number.isNaN(sellPriceMin)) {
      filter.sellPrice = { [Op.gte]: sellPriceMin };
    }
    if (sellPriceMax && !Number.isNaN(sellPriceMax)) {
      filter.sellPrice = { ...filter.sellPrice, [Op.lte]: sellPriceMax };
    }
    // Filtro average Score
    if (averageScoreMin && !Number.isNaN(averageScoreMin)) {
      filter.averageScore = { [Op.gte]: averageScoreMin };
    }
    if (averageScoreMax && !Number.isNaN(averageScoreMax)) {
      filter.averageScore = {
        ...filter.averageScore,
    ...
    if (pagesMin && !Number.isNaN(pagesMin)) {
      filter.pages = { [Op.gte]: pagesMin };
    }
    if (pagesMax && !Number.isNaN(pagesMax)) {
      filter.pages = { ...filter.pages, [Op.lte]: pagesMax };
    }
    // Filtro stock
    if (stockMin && !Number.isNaN(stockMin)) {
      filter.stock = { [Op.gte]: stockMin };
    }
    if (stockMax && !Number.isNaN(stockMax)) {
      filter.stock = { ...filter.stock, [Op.lte]: stockMax };
    }
  ...
  try {
    ...
    if (typeof author == "string") {
      ...
      if (!author || author.length === 1) {
        const existentAuthor = await Author.findOne({
          where: { name: "Unknown" },
        });
        if (existentAuthor) {
        ...
        if (!existentAuthor) {
          let newDBAuthor = await Author.create({
            id: createdAuthorsInDbId,
            name: "Unknown",
          });
          if (newDBAuthor) {
    ...
    if (genre.length === 0) {
      ...
      if (existentGenre) {
        genreId.push(existentGenre.dataValues.id);
      }
      // Si no existe el "unknown" en la DB, lo creo y asigno su id para la relacion
      // Esta funcion se ejecuta una unica vez hasta que se haga un force en la DB
      if (!existentGenre) {
        let newDBGenre = await Genre.create({
          id: createdGenresInDbId,
          name: "Unknown",
        });
        if (newDBGenre) {
          genreId.push(newDBGenre.dataValues.id);
        }
      }
    }
    ...
    if (genre) {
      ...
      if (numbers.length > 0) {
        const existentGenre = await Genre.findAll({
          where: { id: { [Op.in]: numbers } },
        });
        if (existentGenre) {
...
const isForSaleController = async (id, percentageDiscount) => {
  try {
    const book = await Book.findOne({ where: { id: id } });
    ...
    const discountedPrice =
      book.sellPrice - (book.sellPrice * percentageDiscount) / 100;
