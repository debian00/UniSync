...
const getReviewByUserIdController = async (userId) => {
  try {
    ...
    const bookIds = review.map((saleItem) => saleItem.bookId);

    const booksInReview = await Book.findAll({
      where: {
        id: {
          [Op.in]: bookIds,
        },
    ...
    const saleWithBooks = review.map((reviewItem) => {
      const bookData = booksInReview.find(
        (book) => book.id === reviewItem.bookId
      );
      return {
        ...reviewItem.dataValues,
        book: bookData,
      };
    });
...
const createReviewController = async ({ score, comment, userId, bookId }) => {
  try {
    //! Creo mi review
...
const updateReviewController = async (id, { score, comment, bookId }) => {
  try {
